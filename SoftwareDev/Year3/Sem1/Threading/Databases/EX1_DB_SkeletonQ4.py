...
def doubleAgeCmd(ageLimit):
    try:
        # cur.execute("select * from Person where Age <= "+ str(ageLimit))
        cur.execute("Update Person set Age=Age*2 where Age <= " + str(ageLimit))
        con.commit()
        # list = cur.fetchall()
        #
        # for currentPers in list:
        #     cur.execute("Update Person set Age=Age + Age where Name = \'" + currentPers[0] + "\'")
        #     print("[", currentPers[0], "\t", currentPers[1], "]")
    except:
