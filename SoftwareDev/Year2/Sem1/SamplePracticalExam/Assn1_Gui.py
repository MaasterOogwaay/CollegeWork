...
def goalsCmd():
    try:
        ...
        entryMsg.insert(END, "No Games Played Yet")
def percentAttended():
    try:
...
def insertNewStudent():
    ...
    if (var_cb1.get()==1):
        PremierLeague=True
    newTeam=Team(name, Nationality, PremierLeague)
...
labelMsg = Label(frame, text="Error Message", fg="Red", width=15, font=("arial", 11, "bold"))  #
labelMsg.grid(row=14, column=0, columnspan=2, sticky=W )

entryMsg = Entry(frame)
entryMsg.insert(END, '')
entryMsg.grid(row=14, column=1, sticky=W+E)
