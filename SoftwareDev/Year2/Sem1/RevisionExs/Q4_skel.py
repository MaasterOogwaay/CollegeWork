...
def searchEvent():
    ...
    result = noneGreater(list1, target)
    entry4.delete(0, END)
    if result == True:
...
def largestEvent():
    global list1
    target = int(entry3.get())
    result = countLessThan(list1, target)
    entry5.delete(0, END)
...
button1 = Button(window, text="noneGreater", width=12, fg="black", font=("arial", 10, "bold"), command=searchEvent)
...
button2 = Button(window, text="count < Target", width=12, fg="black", font=("arial", 10, "bold"), command=largestEvent)
